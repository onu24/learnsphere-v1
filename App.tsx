...
import { User, CartItem, UserRole, AuthState, Course, Transaction, OrderStatus, EnrolledCourse } from './types';
...
const Hero: React.FC = () => {
  ...
  return (
    <div 
      className="relative h-[500px] w-full overflow-hidden bg-slate-900"
        ...
        <div className="relative overflow-hidden h-48">
          ...
          <div className="absolute top-4 left-4 z-20">
            <button
              ...
              type="button"
              className={`p-2 rounded-full transition-all duration-200 hover:scale-110 active:scale-95 bg-white/95 backdrop-blur-sm shadow-md cursor-pointer ${isInWishlist(course.id)
            ...
            >
              <Heart
                size={20}
                className={`transition-all ${isInWishlist(course.id) ? 'fill-current scale-110' : ''}`}
              />
            </button>
          </div>
...
const Payment: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 3. ENROLL USER IN PURCHASED COURSES (if user is logged in and transaction is confirmed)
      if (user && isVerified) {
        try {
          // Get all courses to match by name
          const allCourses = await DB.getCourses();
          
          // Enroll user in each purchased course
          for (const cartItem of items) {
            // Find the course by ID (cart items have full course data)
            const course = allCourses.find(c => c.id === cartItem.id) || cartItem;
            
            // Create enrolled course data
            ...
            // Enroll the user
            await DB.enrollInCourse(user._id, enrolledCourse);
          }
        } catch (enrollError) {
          console.error('Failed to enroll in courses:', enrollError);
          // Don't fail the transaction if enrollment fails, just log it
        }
      }
