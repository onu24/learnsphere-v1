...
import { Course, User, OrderStatus } from '../types';
...
export const AdminDashboard: React.FC = () => {
    ...
    // Refresh stats when courses change
    useEffect(() => {
        loadStats();
    }, [courses.length]);
    ...
    const loadStats = async () => {
        try {
            ...
            // Calculate total students (users who have enrolled in at least one course)
            const studentsWithCourses = regularUsers.filter(user => 
                user.enrolledCourses && user.enrolledCourses.length > 0
            );
            // If no enrolled courses, count all regular users as students
            setTotalStudents(studentsWithCourses.length > 0 ? studentsWithCourses.length : regularUsers.length);
            
            // Get all transactions
            const transactions = await DB.getTransactions();
            
            // Calculate total revenue from confirmed transactions
            // Using OrderStatus.CONFIRMED enum value (which equals 'Confirmed')
            const confirmedTransactions = transactions.filter(tx => tx.status === OrderStatus.CONFIRMED);
            const revenue = confirmedTransactions.reduce((sum, tx) => sum + (tx.totalAmount || 0), 0);
            setTotalRevenue(revenue);
            
            // Calculate active users (users who have made purchases or enrolled in courses)
            const activeUserIds = new Set<string>();
            confirmedTransactions.forEach(tx => {
                if (tx.userId) activeUserIds.add(tx.userId);
            });
            ...
            // If no active users found, use total regular users
            setActiveUsers(activeUserIds.size > 0 ? activeUserIds.size : regularUsers.length);
        } catch (error) {
            ...
            try {
                ...
                const transactions = await DB.getTransactions();
                const regularUsers = users.filter(u => u.role === 'user');
                setTotalStudents(regularUsers.length);
                const confirmedTxs = transactions.filter(tx => tx.status === OrderStatus.CONFIRMED);
                setTotalRevenue(confirmedTxs.reduce((sum, tx) => sum + (tx.totalAmount || 0), 0));
                setActiveUsers(regularUsers.length);
            } catch (fallbackError) {
                console.error('Fallback stats loading failed:', fallbackError);
                // Set to 0 if everything fails
                setTotalStudents(0);
                setTotalRevenue(0);
                setActiveUsers(0);
            }
